    ...
    // Prefill bookings this week so the home preview isn't empty on first login
    (function seedBooking() {
      const { start, end } = weekBounds();
      const candidates = state.slots.filter((s) => {
      ...
      if (!picks.length) {
        const fallback = state.slots.find((s) => s.status === 'available');
    ...
    function persist() {
      sessionStorage.setItem('sb-demo-old', JSON.stringify({
        authed: state.authed,
        role: state.role,
    ...
    function restore() {
      try {
        const raw = sessionStorage.getItem('sb-demo-old');
        if (!raw) return;
    ...
    function bookSlot(slotId) {
      ...
      if (!slot) {
        showFlash('El turno ya no está disponible.');
        render();
        return;
      }
      slot.status = 'booked';
    ...
    async function cancelBooking(id) {
      ...
      persist();
      showFlash('Turno cancelado. El horario volvió a estar disponible.');
      render();
    }
