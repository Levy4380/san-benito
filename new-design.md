...
- **d21 — Alta manual del vínculo**: el doctor solo vincula pacientes **a sí mismo**; admin/super_admin puede vincular **cualquier paciente a cualquier doctor**. Búsqueda por nombre, DNI o correo entre pacientes registrados; **mínimo 2 caracteres**. Par duplicado: idempotente.
...
- **d36 — Catálogos admin (chrome unificado)**: `/admin/appointments`, `/admin/doctors`, `/admin/admins`, `/admin/patients`, `/admin/settings` y `/admin/settings/specialties` usan el recuadro `center-stage` → `doctor-results` (filtros **dentro**, como `#page-doctors`). **Patrón de filtros (d41)**: toolbar con un campo primario + Buscar (+ Filtros si hay avanzados). En Doctores el primario es nombre y el panel lleva especialidad (incluye “Todas”, **predeterminada**, d18); en Administradores / Pacientes solo el campo de texto (sin Filtros). **Sin filtros se lista todo**. Reservas: primario = fecha; panel = doctor, paciente, especialidad (incluye “Todas”, **predeterminada**). El alta (d35) es otra GET con el mismo chrome; no reemplaza la lista en la misma URL. Card de doctor en `/admin/doctors`: CTA **Más información** → `#page-doctor-profile` (`/doctors/{id}`); la card no navega al click. Cards de staff en Administradores no. Cards de paciente en `/admin/patients`: CTA **Más información** → perfil admin (`/admin/patients/{patient}`).
- **d41 — Toolbar + panel de filtros**: en listados con `Filters`/`Catalog` (Reservas, Doctores portal/admin, Pacientes, Administradores, Mis pacientes) la StageCard no apila todos los filtros en la banda. **Lista:** un solo campo primario (texto o fecha) + **Buscar**; si hay filtros avanzados, botón **Filtros**. **Filtros** reemplaza el contenido de la StageCard por el formulario avanzado. **Aplicar** busca (primario actual + avanzados) y vuelve a la lista. **Volver** cierra sin aplicar y descarta ediciones del panel. Sin avanzados: solo primario + Buscar.
...
| `#page-doctors` | Doctores | Filtros **dentro** del recuadro (d41): primario nombre + Buscar; panel **Filtros** con especialidad (arranca en “Todas”). Se listan todos con ≥1 especialidad (d18). Cards: nombre, especialidades, **Más información** (perfil) y **Ver turnos**. La card **no** navega al click. | Sí |
...
3. **Recuadro** `center-stage` → `doctor-results`: filtros **dentro** con toolbar + panel (d41). Mismo recuadro en catálogos admin (d36).
4. **`agenda`**: `cal` + `panel`.
...
16. Doctor ve solo sus pacientes; búsqueda por nombre, DNI o correo.
