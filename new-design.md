...
- **d36 — Catálogos admin (chrome unificado)**: `/admin/appointments`, `/admin/doctors`, `/admin/admins`, `/admin/patients`, `/admin/settings` y `/admin/settings/specialties` usan el recuadro `center-stage` → `doctor-results` (filtros **dentro**, como `#page-doctors`). En Doctores se filtra por especialidad (incluye “Todas”, **predeterminada**, d18) y/o nombre; en Administradores se filtra por nombre o correo en un solo campo; en Pacientes por nombre, DNI o correo en un solo campo. **Sin filtros se lista todo**. Reservas conservan doctor / paciente / fecha. El alta (d35) es otra GET con el mismo chrome; no reemplaza la lista en la misma URL. Card de doctor en `/admin/doctors`: CTA **Más información** → `#page-doctor-profile` (`/doctors/{id}`); la card no navega al click. Cards de staff en Administradores no. Cards de paciente en `/admin/patients`: CTA **Más información** → perfil admin (`/admin/patients/{patient}`).
