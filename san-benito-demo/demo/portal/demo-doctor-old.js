(() => {
  const STORAGE = 'sb-demo-doctor-old';
  const DOCTOR = {
    id: 1,
    name: 'Dra. Ana Pérez',
  ...
  function commitFranja(days, start, end) {
    ...
    showFlash(`Listo: ${created} turno(s)${preview.skipped ? ` · ${preview.skipped} omitido(s)` : ''}.`);
    closeProgramFlow();
    persist();
  }
  ...
  async function cancelSlot(id) {
    ...
    slot.patientId = null;
    showFlash('Turno cancelado. El slot volvió a disponible.');
    persist();
    render();
  }
  ...
  function renderSettings() {
    ...
    body.innerHTML = `
      ...
      <p class="hint" style="margin-top:auto">Para cargar franjas: <button type="button" class="back-link" id="goto-program" style="display:inline;padding:0">Programar turnos →</button>.</p>
    `;
